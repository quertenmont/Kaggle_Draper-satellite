   ...
   while(True):
	   ...
	   for i in range(0,len(imgDetails)):
		 ...
		 if(i==j):continue
                 ...
                 #check if this was already assessed           
                 alreadyIn = False
                 for match in Matches:
                    if(match['i'] == i and match['j'] == j):
                       alreadyIn = True
                       break
                 if(alreadyIn):continue #no need to recompute

                 print("check %i vs %i" % (i,j))
	...
	#        print matches
	   ...
	   for match in Matches:
   	      #compute Homography matrix for the best match
              print "test %i - %i  size=%i" % (match['i'], match['j'], len(imgDetails) )
	      ...
	      det = np.linalg.det(M[0:2,0:2])
           ...
           print "BEST %i - %i" % (bestMatch['i'], bestMatch['j'])          
	   ...
	   M = M+np.float32([[0,0,-1*x],[0,0,-1*y],[0,0,0]])  #+Moffset;
           ...
           #recompute features for merged picture
           A['kp'], A['des'] = orb.detectAndCompute(A['imggrey'],None)


           #remove matches corresponding to index i or j, and reindex the match list
           #this allows to not reocmpute pairs that haven't changed
           m=0
           while(m<len(Matches)):
              match = Matches[m]
              if(match['i'] == bestMatch['i'] or match['i'] == bestMatch['j'] or match['j'] == bestMatch['i'] or match['j'] == bestMatch['j']):
                 Matches.remove(match)
                 continue

              if(match['i'] > bestMatch['j']): match['i'] = match['i']-1
              if(match['j'] > bestMatch['j']): match['j'] = match['j']-1
              m = m+1


           cv2.imshow('merged', A['imggrey'])
           cv2.waitKey(10)
...
for imagePath in imagePathList: 
   ...
   images[-1] = cv2.resize  (images[-1], (images[-1].shape[1]/8, images[-1].shape[0]/8)) #reduce size
